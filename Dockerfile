...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends libfuse2 openscad lxterminal nano wget openssh-client rsync ca-certificates xdg-utils htop tar xzip gzip bzip2 zip unzip && \
    rm -rf /var/lib/apt/lists
